...
import { useFiltersStore } from "@/app/providers/ZustandStore";

export const Filters: React.FC = () => {
  const { city, minPrice, maxPrice, minRating, sort, setFilters } = useFiltersStore();
  const [local, setLocal] = useState({ city, minPrice, maxPrice, minRating, sort });

  useEffect(() => setLocal({ city, minPrice, maxPrice, minRating, sort }), [city, minPrice, maxPrice, minRating, sort]);
  ...
  function reset() {
    const defaultState = { city: "", minPrice: 0, maxPrice: 9999, minRating: 0, sort: "price_asc" };
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input 
            ...
            onChange={(e) => setLocal({ ...local, minRating: Number(e.target.value) })} 
            className="w-full mt-1 p-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
          />
        </div>
