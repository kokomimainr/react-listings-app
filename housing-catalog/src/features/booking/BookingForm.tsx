...
export const BookingForm: React.FC<BookingFormProps> = ({ listingId, pricePerNight }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md border">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {calculateTotal() > 0 && (
          <div className="border-t pt-4">
            <div className="flex justify-between text-sm mb-1">
              <span>${pricePerNight} × {Math.ceil((new Date(checkOut).getTime() - new Date(checkIn).getTime()) / (1000 * 60 * 60 * 24))} nights</span>
        ...
        >
          {isPending ? "Booking..." : token ? "Book Now" : "Login to Book"}
        </button>
      </form>
    </div>
  );
